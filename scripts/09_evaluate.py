...
def main() -> None:
    ...
    report = evaluate_outputs(
        candidates, links, alignments, variants, linked_resolved, family
    )
    ...
    write_manifest(
        ...
        stage="09_evaluate",
        inputs=[*candidates, links, alignments, variants, linked_resolved, family],
        outputs=[output],
        row_counts={"metrics": len(report)},
        parameters={"state_metrics_weighted_by": "n_times"},
    )
